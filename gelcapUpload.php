<?php
	...
	if (($_FILES["file"]["size"] > 0) && ($_FILES["file"]["size"] < 20000) && in_array($extension, $allowedExts)) //Check that file is 0-20000 bytes and has an acceptable extension
	{
		...
		else
		{
			...
			{
				$fw = fopen("upload/gelName.txt", "w");	//Store original name of file in text document
				fwrite($fw, $_FILES["file"]["name"]);
				fclose($fw);
				move_uploaded_file($_FILES["file"]["tmp_name"], "upload/gelcapData.txt");	//Move the file to the upload folder
			}
		}
	}
	else{
